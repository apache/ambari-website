...
## 实用 Docker 快捷方式 {#useful-docker-shortcuts}

### 快速访问容器 {#quick-container-access}

为便于访问 Docker 容器，可以向系统添加一个 Shell 函数。该函数让你通过简单命令快速进入任意容器。

将以下函数添加到 `/etc/profile`（需要 root 访问权限）：
...
此快捷方式避免每次访问容器时都输入完整的 `docker exec -it container_name /bin/bash` 命令。
